...
#define FT_PAGE_MIN         20
    ...
    {   
        ...
        PRINT("want page:%zd, max continue:%zd", wantpages, m_iMaxContinuePages);
        ...
        PRINT("want page:%zd, bestIndex:%zd", wantpages, bestIndex);
        if (pageInfo->page_count == wantpages) {
            ...
            PRINT("after alloc, address tree size:%zd", m_iAddressTreeSize);
        } else {
        ...
        PRINT("want page:%zd, max continue:%zd, ret:%p", wantpages, m_iMaxContinuePages, addr);
        return addr;
    }
    ...
    void CPageMgr::ReleasePageIfNeed()
    {
        ...
        PRINT("freepages:%zd, mmap:%zd, sbrk:%zd, idle rate:%d\n",
            m_iFreePages, m_iMmapPages, m_iSbrkPages, idleRate);
            
        if ((m_iMmapPages + m_iSbrkPages) < FT_PAGE_MIN || idleRate < FT_PAGE_IDLE_RATE) {
            PRINT("no need release pages!, idlerate:%d, min_pages:%d",
                FT_PAGE_IDLE_RATE, FT_PAGE_MIN);
            return;
        }

        size_t freesize = m_iFreePages * FT_PAGE_IDLE_RATE / 100;
        PRINT("need release %d pages!", freesize);
        
        if (freesize <= 0) {
            return;
        }

        void * brk_addr = (void *)(m_llTopBrkAddress - 1);
        struct SPageInfo * pageInfo = (struct SPageInfo *)GetPageInfoByAddress(brk_addr);
        if (pageInfo == NULL) {
            PRINT("sbrk top address is inuse,  can't brk down!");
        }
        PRINT("sbrk top info, addr:%p, pagecount:%zd", (void*)pageInfo->base_address, pageInfo->page_count);
        size_t sbrk_freesize = pageInfo->page_count - freesize;
    }
    ...
    int CPageMgr::ReleasePagesToSys(struct SPageInfo * pageInfo)
    {
        void * addr = (void *)pageInfo->base_address;
        size_t size = pageInfo->page_count << FT_PAGE_BIT;
        
        if (pageInfo->FromBrk()) {
            s_sbrk_page_allocator.SysRelease(addr, size);
            m_iSbrkPages -= pageInfo->page_count;
        } else {
            s_mmap_page_allocator.SysRelease(addr, size);
            m_iMmapPages -= pageInfo->page_count;
        }
    }
